...
export class LoginService {
  ...
  async login(dto: LoginDto) {
    ...
    if (user.status === 'BANNED')
      return errorResponse(400, 'Tài khoản đã bị khóa', 'BANNED');
    if (user.status === 'DELETED')
      return errorResponse(400, 'Tài khoản đã bị xóa', 'DELETED');
    const verify = await compare(dto.password, user.password);
    if (!verify)
      return errorResponse(400, 'Mật khẩu không đúng', 'INCORRECT_PASSWORD');
