...
export class GetEnterprisesMapController {
  constructor(
    private readonly getEnterprisesMapService: GetEnterprisesMapService,
  ) {}
  ...
  @ApiQuery({
    name: 'status',
    required: false,
    enum: ['ACTIVE', 'PENDING', 'BANNED', 'EXPIRED', 'OFFLINE'],
  })
  ...
  async getEnterpriseDetailMap(@GetUser() user: User, @Param('id') id: number) {
