...
export class CreateReportService {
   ...
   * - ĐƯỢC HỦY khi: PENDING, ENTERPRISE_RESERVED, COLLECTOR_PENDING
   * - KHÔNG ĐƯỢC HỦY khi: ON_THE_WAY, ARRIVED, COLLECTED, REJECTED, CANCELLED
   */
  async cancelReport(
    citizenId: number,
    reportId: number,
    ...
    try {
      ...
      if (report.citizenId !== citizenId) {
        return errorResponse(
          403,
          'Bạn không có quyền hủy báo cáo này',
          'FORBIDDEN',
        );
      }
      ...
      if (!cancellableStatuses.includes(report.status)) {
        return errorResponse(
          400,
          `Trạng thái "${report.status}" không cho phép hủy`,
          'INVALID_STATUS',
        );
      }
      ...
      this.logger.log(
        `Citizen ${citizenId} cancelled report ${reportId}. Status was: ${report.status}`,
      );
