...
import {
  GetCollectorsQueryDto,
  CollectorAvailabilityFilter,
} from '../dtos/get-collectors-query.dto';
...
export class GetAllCollectorService {
  ...
  async getAllCollectors(userId: number, query: GetCollectorsQueryDto) {
    try {
      ...
      if (query.status) {
        where.status = { availability: query.status };
      }
      ...
      const items = collectors.map((c) => ({
        ...
        status: c.status?.availability || 'OFFLINE',
        primaryZone: c.primaryZone,
      ...
      return errorResponse(
        500,
        'Lỗi khi lấy danh sách collector',
        error.message,
      );
    }
  }
