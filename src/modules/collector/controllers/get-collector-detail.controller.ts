...
export class GetCollectorDetailController {
  constructor(
    private readonly getCollectorDetailService: GetCollectorDetailService,
  ) {}
  ...
  async getCollectorDetail(@GetUser() user: User, @Param('id') id: number) {
    return this.getCollectorDetailService.getCollectorDetail(
      user.id,
      Number(id),
    );
  }
