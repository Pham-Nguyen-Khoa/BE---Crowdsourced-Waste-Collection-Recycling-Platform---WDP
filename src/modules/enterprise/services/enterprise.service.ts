...
import { ZoneService } from 'src/modules/zone/services/zone.service';
...
export class EnterpriseService {
  ...
  async registerAndCreatePayment(userId: number, dto: CreateEnterpriseDto) {
    const existingEnterprise =
      await this.enterpriseRepository.findEnterpriseByUserId(userId);
    if (existingEnterprise) {
      return errorResponse(
        400,
        'Tài khoản đã đăng ký doanh nghiệp',
        'ALREADY_ENTERPRISE',
      );
    }

    try {
      const enterprise = await this.enterpriseRepository.registerEnterprise(
        userId,
        dto,
      );
      ...
      (enterprise as any).subscriptionPlanConfigId =
        dto.subscriptionPlanConfigId;

      const payment = await this.enterpriseRepository.createPayment(
        userId,
        paymentDto,
      );
  ...
  async createPayment(userId: number, dto: CreatePaymentDto) {
    try {
      const existingPendingPayment =
        await this.enterpriseRepository.findPaymentByEnterpriseId(
          dto.enterpriseId,
        );

      if (
        existingPendingPayment &&
        existingPendingPayment.status === 'PENDING'
      ) {
        if (
          existingPendingPayment.expiresAt &&
          existingPendingPayment.expiresAt > new Date()
        ) {
          return errorResponse(
            400,
            'Doanh nghiệp đã có thanh toán đang chờ xử lý',
            'PAYMENT_EXISTS',
          );
        }

        await this.enterpriseRepository.cancelPayment(
          existingPendingPayment.referenceCode,
        );
      }
      ...
      if (error.message.includes('not found')) {
        return errorResponse(
          404,
          'Doanh nghiệp không tồn tại hoặc không thể kích hoạt',
  ...
  async getPayment(referenceCode: string, userId?: number) {
    const payment =
      await this.enterpriseRepository.findPaymentByReferenceCode(referenceCode);
    if (!payment) {
      return errorResponse(
        400,
  ...
  async cancelPayment(referenceCode: string, userId: number) {
    ...
    if (payment.userId !== userId) {
      return errorResponse(
        403,
        'Không có quyền hủy thanh toán này',
        'FORBIDDEN',
      );
    }
    ...
    if (payment.status !== 'PENDING') {
      return errorResponse(
        400,
        'Không thể hủy thanh toán đã hoàn tất',
        'CANNOT_CANCEL',
      );
    }
    ...
    try {
      ...
      if (enterprise && enterprise.status === 'PENDING') {
        ...
        console.log(
          `Deleted pending enterprise ${enterprise.id} when canceling payment ${referenceCode}`,
        );
      }
      ...
      return successResponse(
        200,
        null,
        'Hủy thanh toán và đăng ký doanh nghiệp thành công',
      );
    } catch (error) {
  ...
  async getSubscriptionPlans() {
    try {
      const plans =
        await this.enterpriseRepository.findActiveSubscriptionPlans();
  ...
  async processSePayWebhookRaw(webhookData: any) {
    ...
    const payment =
      await this.enterpriseRepository.findPaymentByReferenceCode(
        paymentReferenceCode,
      );
    if (!payment) {
      console.log(
        `Payment not found for reference code: ${paymentReferenceCode}`,
      );
      return errorResponse(
        404,
    ...
    if (Number(payment.amount) !== Number(webhookAmount)) {
      console.log(
        `Amount mismatch: payment=${Number(payment.amount)}, webhook=${webhookAmount}`,
      );
    ...
    const result =
      await this.enterpriseRepository.activateEnterprise(paymentReferenceCode);
    ...
    try {
      ...
      if (!enterprise) {
        return successResponse(200, {
          ...
          message: 'Chưa đăng ký doanh nghiệp',
        });
      }
  ...
  async resumeRegistration(userId: number) {
    try {
      ...
      const subscriptionPlanConfigId =
        (enterprise as any).subscriptionPlanConfigId ||
        pendingPayment.subscriptionPlanConfigId;
  ...
  async retryPayment(userId: number, enterpriseId: number) {
    try {
      ...
      const existingPayment =
        await this.enterpriseRepository.findPaymentByEnterpriseId(enterpriseId);
      if (existingPayment && existingPayment.status === 'PENDING') {
        await this.enterpriseRepository.cancelPayment(
          existingPayment.referenceCode,
        );
      }
  ...
  async updateEnterprise(userId: number, dto: UpdateEnterpriseDto) {
    try {
      ...
      const updatedEnterprise =
        await this.enterpriseRepository.updateEnterprise(enterprise.id, {
          name: dto.name,
          address: dto.address,
      ...
      if (dto.serviceAreas && dto.serviceAreas.length > 0) {
        await this.enterpriseRepository.deleteEnterpriseServiceAreas(
          enterprise.id,
        );
        await this.enterpriseRepository.createEnterpriseServiceAreas(
          enterprise.id,
          dto.serviceAreas,
        );

        // Revalidate zones after service area change
        await this.zoneService.revalidateZones(enterprise.id);
      }

      // Cập nhật waste types nếu có
      if (dto.wasteTypes && dto.wasteTypes.length > 0) {
        await this.enterpriseRepository.deleteEnterpriseWasteTypes(
          enterprise.id,
        );
        await this.enterpriseRepository.createEnterpriseWasteTypes(
          enterprise.id,
          dto.wasteTypes.map((wt) => wt.wasteType as any),
        );
      }

      // Lấy lại dữ liệu mới nhất
      const finalEnterprise =
        await this.enterpriseRepository.findEnterpriseByUserId(userId);
      if (!finalEnterprise) {
        return errorResponse(
          500,
          'Không tìm thấy enterprise sau khi cập nhật',
          'NOT_FOUND',
        );
      }
  ...
  async getAllWaitingReports(userId: number) {
    ...
    const reports = waitingAttempts
      .filter((attempt) => attempt.report && attempt.report.deletedAt === null)
      .map((attempt) => ({
        ...attempt.report,
  ...
  async getAcceptedReports(userId: number) {
    ...
    return successResponse(
      200,
      acceptedReports,
      `Lay thanh cong ${acceptedReports.length} bao cao da duoc doanh nghiep chap nhan`,
    );
  }
}
