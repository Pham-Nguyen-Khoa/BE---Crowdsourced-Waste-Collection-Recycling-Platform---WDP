...
export class EnterpriseScheduler {
  ...
  async handleExpiredPayments() {
    try {
      const expiredPayments =
        await this.enterpriseRepository.findExpiredPayments();

      for (const payment of expiredPayments) {
        await this.enterpriseRepository.markPaymentAsFailed(
          payment.referenceCode,
        );
        this.logger.log(
          `Payment ${payment.referenceCode} marked as FAILED (expired)`,
        );
      }
  ...
  async cleanupPendingEnterprises() {
    try {
      const oldPendingEnterprises =
        await this.enterpriseRepository.findOldPendingEnterprises();

      for (const enterprise of oldPendingEnterprises) {
        await this.enterpriseRepository.deleteEnterprise(enterprise.id);
        this.logger.log(
          `Deleted old pending enterprise: ${enterprise.name} (ID: ${enterprise.id})`,
        );
      }

      if (oldPendingEnterprises.length > 0) {
        this.logger.log(
          `Cleaned up ${oldPendingEnterprises.length} old pending enterprises`,
        );
      }
    } catch (error) {
