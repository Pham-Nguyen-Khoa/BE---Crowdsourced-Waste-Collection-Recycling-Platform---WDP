...
export class EnterpriseController {
  ...
  async registerAndCreatePayment(
    @GetUser() user: User,
  ...
  @UseGuards(JWTGuard)
  @Get(
    routesV1.enterprise.getPayment.replace(':referenceCode', ':referenceCode'),
  )
  async getPayment(
...
<<<<<<< HEAD
  ...
  @Permissions(PermissionCode.REGISTER_ENTERPRISE)
  @Delete(
    routesV1.enterprise.cancelPayment.replace(
      ':referenceCode',
      ':referenceCode',
    ),
  )
...
>>>>>>> 630cc8dbeec8327299ddff79c4347b2b48f388ea
  ...
  @ApiOperation({ summary: resourcesV1.GET_ACCEPTED_REPORTS.displayName })
  @ApiBearerAuth()
  @UseGuards(JWTGuard, RolesGuard)
  @Roles(2)
  @Get(routesV1.enterprise.getAcceptedReports)
  async getAcceptedReports(@GetUser() user: User) {
    return await this.enterpriseService.getAcceptedReports(user.id);
  }
